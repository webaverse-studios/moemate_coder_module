...
async function callModel(newMessages = []) {
  ...
  // const responseObj = extractCode(responseContent);
  ...
  window.addEventListener('message', function(event) { // todo: removEventListener
    ...
    } else if (event.data.type === 'IFRAME_INFO') {
      lastConsoleLog = event.data.data;
      console.log('--- lastConsoleLog:', lastConsoleLog);
    }
  });
  ...
  iframeDocument.open();
  iframeDocument.write(scriptBefore + responseObj.html + scriptAfter);
  iframeDocument.close();
  
  setTimeout(async () => { // ensure the triggering of hack_delay. // todo: Prmoise.all // todo: don't await above
    window.hooks.emit("hack_delay", `Tell user the result is ${lastConsoleLog}`);
  }, 100);
...
// You can ONLY use free resources, MUST NOT use src/url or api which includes "token", "ACCESS_TOKEN", "API_KEY", "YOUR_API_KEY" etc.
  ...
  */
  ...
  const responseObj = await callModel(newMessages);
